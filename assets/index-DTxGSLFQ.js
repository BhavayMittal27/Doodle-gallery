(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const d of n)if(d.type==="childList")for(const f of d.addedNodes)f.tagName==="LINK"&&f.rel==="modulepreload"&&s(f)}).observe(document,{childList:!0,subtree:!0});function o(n){const d={};return n.integrity&&(d.integrity=n.integrity),n.referrerPolicy&&(d.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?d.credentials="include":n.crossOrigin==="anonymous"?d.credentials="omit":d.credentials="same-origin",d}function s(n){if(n.ep)return;n.ep=!0;const d=o(n);fetch(n.href,d)}})();const N=[{id:"doodle-weather-app",title:"SkySketch: Weather UI Mockup",category:"portfolio",tag:"UI Wireframe",date:"May 2026",likes:42,author:"Bhavay",description:"A playful, hand-drawn wireframe mockup of a weather forecasting app. Shows layout planning for responsive widget placement, cartoonish weather condition drawings, and simple typography hierarchy. Designed during initial prototyping stages.",techTags:["Figma Sketch","Mobile Design","SVG Layouts","UI/UX Prototyping"],svg:`
  ...
  `},q={frontend:[{name:"JavaScript (ES6+)",value:92},{name:"HTML5 Canvas & SVGs",value:88},{name:"React.js / Next.js",value:85},{name:"CSS3 & Custom Animations",value:95}],backend:[{name:"Node.js & Express",value:80},{name:"SQL & NoSQL Databases",value:75},{name:"REST / GraphQL APIs",value:84},{name:"Git & Web Performance",value:82}]},Q=[{time:"2024 - Present",role:"Senior Full-Stack Architect",company:"CreativeCode Studios",desc:"Spearheaded interactive graphics initiatives. Built high-traffic single page apps utilizing HTML5 Canvas dashboards, cutting asset load-times by 40%. Managed teams of 4 developers."},{time:"2022 - 2024",role:"Web Application Developer",company:"PixelPerfect Solutions",desc:"Crafted modular frontend components for large-scale SaaS applications using React and Next.js. Engineered interactive mockups and custom SVG filters for custom branding campaigns."},{time:"2021 - 2022",role:"Junior UI Engineer",company:"Scribble Web Labs",desc:"Worked closely with design agencies to translate wireframes and doodles into responsive HTML/CSS structures. Optimized web layout layouts for fluid touch devices."}];window.PORTFOLIO_DOODLES=N;window.RESUME_PROFILE=W;window.RESUME_SKILLS=q;window.RESUME_EXPERIENCE=Q;let _=class{constructor(i,o=!1){this.canvas=document.getElementById(i),this.canvas&&(this.ctx=this.canvas.getContext("2d"),this.isMini=o,this.isDrawing=!1,this.startX=0,this.startY=0,this.currentTool="brush",this.currentShape="free",this.currentColor="#2b2b2b",this.brushSize=6,this.brushOpacity=1,this.history=[],this.historyIndex=-1,this.maxHistory=25,this.previewSnapshot=null,this.init())}init(){this.resizeCanvas(),this.setupListeners(),this.isMini||this.saveHistoryState()}resizeCanvas(){let i=null,o=null;this.canvas.width>0&&this.canvas.height>0&&(i=document.createElement("canvas"),i.width=this.canvas.width,i.height=this.canvas.height,o=i.getContext("2d"),o.drawImage(this.canvas,0,0));const s=this.canvas.parentElement.getBoundingClientRect(),n=window.devicePixelRatio||1;this.canvas.width=s.width*n,this.canvas.height=(this.isMini?220:500)*n,this.ctx.scale(n,n),this.canvas.style.width="100%",this.canvas.style.height=this.isMini?"220px":"500px",this.ctx.lineCap="round",this.ctx.lineJoin="round",i&&o&&this.ctx.drawImage(i,0,0,i.width/n,i.height/n)}setupListeners(){this.canvas.addEventListener("mousedown",o=>this.startDrawing(o)),this.canvas.addEventListener("mousemove",o=>this.draw(o)),this.canvas.addEventListener("mouseup",()=>this.stopDrawing()),this.canvas.addEventListener("mouseleave",()=>this.stopDrawing()),this.canvas.addEventListener("touchstart",o=>{o.preventDefault();const s=o.touches[0];this.startDrawing(s)}),this.canvas.addEventListener("touchmove",o=>{o.preventDefault();const s=o.touches[0];this.draw(s)},{passive:!1}),this.canvas.addEventListener("touchend",()=>this.stopDrawing());let i;window.addEventListener("resize",()=>{clearTimeout(i),i=setTimeout(()=>this.resizeCanvas(),100)})}getMousePos(i){const o=this.canvas.getBoundingClientRect();return{x:i.clientX-o.left,y:i.clientY-o.top}}startDrawing(i){this.isDrawing=!0;const o=this.getMousePos(i);this.startX=o.x,this.startY=o.y,this.ctx.lineWidth=this.brushSize,this.ctx.strokeStyle=this.currentColor,this.ctx.globalAlpha=this.brushOpacity,this.currentTool==="eraser"?(this.ctx.globalCompositeOperation="destination-out",this.ctx.strokeStyle="rgba(0,0,0,1)",this.ctx.lineWidth=this.brushSize*1.5):this.currentTool==="highlighter"?(this.ctx.globalCompositeOperation="source-over",this.ctx.globalAlpha=.35):this.ctx.globalCompositeOperation="source-over",this.currentShape!=="free"?this.previewSnapshot=this.ctx.getImageData(0,0,this.canvas.width,this.canvas.height):(this.ctx.beginPath(),this.ctx.moveTo(this.startX,this.startY),this.ctx.lineTo(this.startX+.1,this.startY),this.ctx.stroke())}draw(i){if(!this.isDrawing)return;const o=this.getMousePos(i);if(this.currentShape==="free")this.ctx.lineTo(o.x,o.y),this.ctx.stroke();else if(this.previewSnapshot&&this.ctx.putImageData(this.previewSnapshot,0,0),this.ctx.beginPath(),this.currentShape==="line")this.ctx.moveTo(this.startX,this.startY),this.ctx.lineTo(o.x,o.y),this.ctx.stroke();else if(this.currentShape==="rect"){const s=o.x-this.startX,n=o.y-this.startY;this.ctx.strokeRect(this.startX,this.startY,s,n)}else if(this.currentShape==="circle"){const s=Math.sqrt(Math.pow(o.x-this.startX,2)+Math.pow(o.y-this.startY,2));this.ctx.arc(this.startX,this.startY,s,0,2*Math.PI),this.ctx.stroke()}}stopDrawing(){this.isDrawing&&(this.isDrawing=!1,this.ctx.closePath(),this.isMini||this.saveHistoryState())}saveHistoryState(){this.historyIndex<this.history.length-1&&(this.history=this.history.slice(0,this.historyIndex+1));const i=this.canvas.toDataURL();this.history.push(i),this.history.length>this.maxHistory?this.history.shift():this.historyIndex++,this.updateUndoRedoButtons()}undo(){this.historyIndex>0&&(this.historyIndex--,this.restoreHistoryState())}redo(){this.historyIndex<this.history.length-1&&(this.historyIndex++,this.restoreHistoryState())}restoreHistoryState(){const i=window.devicePixelRatio||1,o=new Image;o.src=this.history[this.historyIndex],o.onload=()=>{this.ctx.clearRect(0,0,this.canvas.width/i,this.canvas.height/i),this.ctx.drawImage(o,0,0,this.canvas.width/i,this.canvas.height/i),this.updateUndoRedoButtons()}}updateUndoRedoButtons(){const i=document.getElementById("undo-btn"),o=document.getElementById("redo-btn");i&&(i.disabled=this.historyIndex<=0),o&&(o.disabled=this.historyIndex>=this.history.length-1)}clear(){const i=window.devicePixelRatio||1;this.ctx.clearRect(0,0,this.canvas.width/i,this.canvas.height/i),this.isMini||(this.history=[],this.historyIndex=-1,this.saveHistoryState())}getMergedDataURL(i){const o=window.devicePixelRatio||1,s=document.createElement("canvas");s.width=this.canvas.width,s.height=this.canvas.height;const n=s.getContext("2d");if(i){n.fillStyle="#1b221f",n.fillRect(0,0,s.width,s.height),n.fillStyle="rgba(255, 255, 255, 0.02)";for(let d=0;d<s.width;d+=16*o)n.fillRect(d,0,1*o,s.height);for(let d=0;d<s.height;d+=16*o)n.fillRect(0,d,s.width,1*o)}else{n.fillStyle="#fcfbfa",n.fillRect(0,0,s.width,s.height),n.strokeStyle="rgba(186, 218, 255, 0.25)",n.lineWidth=1.5*o;const d=24*o;n.beginPath();for(let f=d;f<s.width;f+=d)n.moveTo(f,0),n.lineTo(f,s.height);for(let f=d;f<s.height;f+=d)n.moveTo(0,f),n.lineTo(s.width,f);n.stroke()}return n.drawImage(this.canvas,0,0),s.toDataURL("image/png")}};window.SketchCanvas=_;const X=null;console.warn("Supabase URL or Anon Key not configured. The gallery will fall back to local portfolio sketches.");window.supabase=X;document.addEventListener("DOMContentLoaded",()=>{let k=[...PORTFOLIO_DOODLES],i="all",o="paper";const s=new SketchCanvas("paint-canvas"),n=new SketchCanvas("hero-mini-canvas",!0),d={paper:[{name:"Graphite",hex:"#2b2b2b"},{name:"Rose Pen",hex:"#ec407a"},{name:"Blue Pen",hex:"#29b6f6"},{name:"Purple Marker",hex:"#ab47bc"},{name:"Green Highlighter",hex:"#9ccc65"},{name:"Red Pen",hex:"#ef5350"}],chalkboard:[{name:"White Chalk",hex:"#f5f6f5"},{name:"Pink Chalk",hex:"#ff8da1"},{name:"Blue Chalk",hex:"#8dcbff"},{name:"Purple Chalk",hex:"#e1b1ff"},{name:"Green Chalk",hex:"#b9f6ca"},{name:"Red Chalk",hex:"#ff8a80"}]};f();function f(){M(),B(),S(),T(),m(),R(),P(),H(),O(),U()}function M(){const t=document.getElementById("theme-toggle"),r=t.querySelector(".sun-icon"),a=t.querySelector(".moon-icon");document.body.className="theme-paper",t.addEventListener("click",()=>{o==="paper"?(o="chalkboard",document.body.className="theme-chalkboard",r.style.display="none",a.style.display="block",y("Switched to Chalkboard (Dark Mode)","info")):(o="paper",document.body.className="theme-paper",r.style.display="block",a.style.display="none",y("Switched to Sketchbook (Light Mode)","info")),S(),I()})}function I(){const t=d[o];s.currentColor=t[0].hex,n.currentColor=t[0].hex;const r=document.getElementById("custom-color");r&&(r.value=t[0].hex);const a=document.getElementById("color-palette"),e=a.querySelector(".color-bubble");e&&(a.querySelectorAll(".color-bubble").forEach(l=>l.classList.remove("active")),e.classList.add("active"))}function B(){const t=document.querySelectorAll(".nav-link"),r=document.querySelectorAll(".page-section"),a={root:null,threshold:.3,rootMargin:"-80px 0px 0px 0px"},e=new IntersectionObserver(l=>{l.forEach(u=>{if(u.isIntersecting){const c=u.target.getAttribute("id");t.forEach(h=>{h.classList.remove("active"),h.getAttribute("href")===`#${c}`&&h.classList.add("active")})}})},a);r.forEach(l=>e.observe(l))}function S(){const t=document.getElementById("color-palette");if(!t)return;t.innerHTML="";const r=d[o];r.forEach((e,l)=>{const u=document.createElement("button");u.className=`color-bubble ${l===0?"active":""}`,u.style.backgroundColor=e.hex,u.title=e.name,u.setAttribute("aria-label",e.name),u.addEventListener("click",()=>{t.querySelectorAll(".color-bubble").forEach(h=>h.classList.remove("active")),u.classList.add("active"),s.currentColor=e.hex;const c=document.getElementById("custom-color");c&&(c.value=e.hex)}),t.appendChild(u)});const a=document.getElementById("custom-color");a&&(a.value=r[0].hex,a.addEventListener("input",e=>{t.querySelectorAll(".color-bubble").forEach(l=>l.classList.remove("active")),s.currentColor=e.target.value}))}function T(){const t=document.getElementById("brush-size"),r=document.getElementById("brush-size-val");t.addEventListener("input",c=>{const h=c.target.value;s.brushSize=h,r.textContent=`${h}px`});const a=document.getElementById("brush-opacity"),e=document.getElementById("brush-opacity-val");a.addEventListener("input",c=>{const h=c.target.value/100;s.brushOpacity=h,e.textContent=`${c.target.value}%`});const l=document.querySelectorAll(".tool-btn");l.forEach(c=>{c.addEventListener("click",()=>{l.forEach(h=>h.classList.remove("active")),c.classList.add("active"),s.currentTool=c.dataset.tool})});const u=document.querySelectorAll(".shape-btn");u.forEach(c=>{c.addEventListener("click",()=>{u.forEach(h=>h.classList.remove("active")),c.classList.add("active"),s.currentShape=c.dataset.shape})}),document.getElementById("undo-btn").addEventListener("click",()=>s.undo()),document.getElementById("redo-btn").addEventListener("click",()=>s.redo()),document.getElementById("clear-btn").addEventListener("click",()=>{s.clear(),y("Canvas cleared!","info")}),document.getElementById("clear-hero-canvas").addEventListener("click",()=>{n.clear(),y("Scratchpad cleared!","info")}),document.getElementById("download-btn").addEventListener("click",()=>{const c=o==="chalkboard",h=s.getMergedDataURL(c),g=document.createElement("a");g.download=`doodle_${Date.now()}.png`,g.href=h,g.click(),y("PNG Downloaded!","success")}),document.getElementById("save-gallery-btn").addEventListener("click",async()=>{const c=document.getElementById("doodle-title"),h=c.value.trim()||"Untitled Doodle",g=o==="chalkboard";document.getElementById("save-gallery-btn").innerHTML;let C=s.getMergedDataURL(g);y(`Successfully posted "${h}" to local gallery!`,"success");const w={id:`user-doodle-${Date.now()}`,title:h,category:"user",tag:"Guest Sketch",date:new Date().toLocaleDateString("en-US",{month:"short",year:"numeric"}),likes:0,author:"Guest Designer",description:"A custom, interactive drawing painted live on the canvas within the Sketch Studio. Saved in local session memory.",techTags:["Canvas API","Mouse/Touch","Session Saved"],imageSrc:C};k.unshift(w),m(),c.value="",s.clear(),document.getElementById("gallery").scrollIntoView({behavior:"smooth"})})}function m(){const t=document.getElementById("gallery-grid");if(!t)return;t.innerHTML="";const r=document.getElementById("gallery-search").value.toLowerCase().trim(),a=k.filter(e=>{const l=i==="all"||e.category===i,u=e.title.toLowerCase().includes(r)||e.description.toLowerCase().includes(r)||e.techTags.some(c=>c.toLowerCase().includes(r));return l&&u});if(a.length===0){t.innerHTML=`
      ...
      `,l.addEventListener("click",c=>{if(c.target.classList.contains("card-like-btn")){c.stopPropagation(),L(e.id,l.querySelector(".likes-number"));return}D(e)}),t.appendChild(l)})}function R(){const t=document.querySelectorAll(".filter-btn");t.forEach(r=>{r.addEventListener("click",()=>{t.forEach(a=>a.classList.remove("active")),r.classList.add("active"),i=r.dataset.filter,m()})}),document.getElementById("gallery-search").addEventListener("input",()=>{m()})}function L(t,r){const a=k.find(e=>e.id===t);if(a&&(a.likes++,r.textContent=a.likes,y(`Liked "${a.title}"!`,"success"),document.getElementById("doodle-lightbox").classList.contains("active"))){const l=document.getElementById("like-count");l&&(l.textContent=a.likes)}}function D(t){const r=document.getElementById("doodle-lightbox"),a=document.getElementById("lightbox-media-container"),e=document.getElementById("lightbox-tag"),l=document.getElementById("lightbox-title"),u=document.getElementById("lightbox-author"),c=document.getElementById("lightbox-date"),h=document.getElementById("lightbox-description"),g=document.getElementById("lightbox-tech-tags"),v=document.getElementById("lightbox-download"),x=document.getElementById("lightbox-like"),C=document.getElementById("like-count");if(a.innerHTML="",t.svg){a.innerHTML=t.svg;const p=new Blob([t.svg],{type:"image/svg+xml;charset=utf-8"});v.href=URL.createObjectURL(p),v.download=`${t.id}.svg`}else t.imageSrc&&(a.innerHTML=`<img src="${t.imageSrc}" alt="${t.title}">`,v.href=t.imageSrc,v.download=`${t.title.replace(/\s+/g,"_")}.png`);e.textContent=t.tag,l.textContent=t.title,u.textContent=t.author,c.textContent=t.date,h.textContent=t.description,C.textContent=t.likes,g.innerHTML="",t.techTags.forEach(p=>{const b=document.createElement("span");b.className="tech-tag hand-drawn",b.textContent=p,g.appendChild(b)});const w=x.cloneNode(!0);x.parentNode.replaceChild(w,x),w.addEventListener("click",()=>{document.querySelector(".gallery-card");const p=document.getElementById("like-count");L(t.id,p),m()}),r.classList.add("active");const G=document.getElementById("close-lightbox-btn"),F=document.getElementById("lightbox-close-overlay"),E=()=>{r.classList.remove("active")};G.onclick=E,F.onclick=E}function P(){const t=document.querySelectorAll(".resume-tab"),r=document.getElementById("resume-tab-content"),a=e=>{r.style.opacity="0",setTimeout(()=>{e==="profile"?A():e==="skills"?z():e==="experience"&&$(),r.style.opacity="1"},150)};t.forEach(e=>{e.addEventListener("click",()=>{t.forEach(l=>l.classList.remove("active")),e.classList.add("active"),a(e.dataset.tab)})}),a("profile")}function A(){const t=document.getElementById("resume-tab-content");t.innerHTML=`
    ...
    `}function z(){const t=document.getElementById("resume-tab-content");t.innerHTML=`
    ...
    `,setTimeout(()=>{t.querySelectorAll(".skill-bar-inner").forEach(r=>{const a=r.getAttribute("data-value");r.style.width=`${a}%`})},50)}function $(){const t=document.getElementById("resume-tab-content");t.innerHTML=`
    ...
    `}function H(){const t=document.getElementById("contact-form"),r=document.getElementById("form-status");t&&t.addEventListener("submit",a=>{a.preventDefault();const e=t.querySelector('button[type="submit"]'),l=e.innerHTML;e.disabled=!0,e.innerHTML="Writing Scribble... ✎",r.textContent="",setTimeout(()=>{y("Message sent! Thanks for leaving a scribble.","success"),e.disabled=!1,e.innerHTML=l,r.innerHTML='<span style="color:var(--ink-success)">✓ Scribble delivered to mailbox!</span>',t.reset()},1500)})}function O(){document.addEventListener("mousemove",t=>{const r=document.querySelectorAll(".floating-doodle"),a=t.clientX/window.innerWidth-.5,e=t.clientY/window.innerHeight-.5;r.forEach((l,u)=>{const c=(u+1)*15,h=a*c,g=e*c;l.style.transform=`translate(${h}px, ${g}px) rotate(${c+h*.2}deg)`})})}function y(t,r="success"){const a=document.getElementById("toast-container");if(!a)return;const e=document.createElement("div");e.className=`toast ${r}`;let l="★";r==="info"&&(l="✎"),r==="danger"&&(l="⚠"),e.innerHTML=`<span class="hand-drawn">${l}</span> <span>${t}</span>`,a.appendChild(e),setTimeout(()=>{e.classList.add("show")},10),setTimeout(()=>{e.classList.remove("show"),setTimeout(()=>{e.remove()},300)},3e3)}async function U(){}});
